...
pub(crate) fn setup_base_sender(
    ...
    // We are not using context generated by the HPKE crate and generate the
    // request key from the exported secret. This is required to send serialized
    // session keys from the kernel to the application.
...
fn seal(
    key: &AeadKey,
    base_nonce: &AeadNonce,
    sequence_number: &mut u128,
    plaintext: &[u8],
    associated_data: &[u8],
) -> anyhow::Result<Vec<u8>> {
