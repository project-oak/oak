...
use std::{boxed::Box, collections::BTreeMap, string::ToString};
...
use oak_session_testing::{
    ...
    create_mock_assertion_generator, create_mock_attester, create_mock_endorser,
    create_mock_session_binding_verifier_provider, create_passing_mock_assertion_verifier,
...
fn self_attested_client_provides_request_accepts_response() -> anyhow::Result<()> {
    let client_config = AttestationHandlerConfig {
        self_attesters: BTreeMap::from([(
            ...
            create_mock_attester().into(),
        )]),
        self_endorsers: BTreeMap::from([(
            MATCHED_ATTESTER_ID1.to_string(),
            create_mock_endorser().into(),
        )]),
        ..Default::default()
    };
...
fn client_with_assertion_generator_provides_request_with_assertion() -> anyhow::Result<()> {
    ...
    let client_config = AttestationHandlerConfig {
        self_assertion_generators: BTreeMap::from([(
            MATCHED_ATTESTER_ID1.to_string(),
            create_mock_assertion_generator(assertion.clone()).into(),
        )]),
        ..Default::default()
    };
    ...
    let client_config = AttestationHandlerConfig {
        peer_verifiers: BTreeMap::from([(
            ...
            PeerAttestationVerifier {
                verifier: create_passing_mock_verifier().into(),
                binding_verifier_provider: create_mock_session_binding_verifier_provider().into(),
            },
        )]),
        peer_assertion_verifiers: BTreeMap::from([(
            MATCHED_ATTESTER_ID1.to_string(),
            create_passing_mock_assertion_verifier(assertion.clone()).into(),
        )]),
...
fn client_failed_evidence_verification_fails() -> anyhow::Result<()> {
    ...
    let client_config = AttestationHandlerConfig {
        peer_verifiers: BTreeMap::from([(
            MATCHED_ATTESTER_ID1.to_string(),
            PeerAttestationVerifier {
                verifier: create_failing_mock_verifier().into(),
                binding_verifier_provider: create_mock_session_binding_verifier_provider().into(),
            },
        )]),
        peer_assertion_verifiers: BTreeMap::from([(
...
fn client_failed_assertion_verification_fails() -> anyhow::Result<()> {
    ...
    let client_config = AttestationHandlerConfig {
        ...
        peer_assertion_verifiers: BTreeMap::from([(
            MATCHED_ATTESTER_ID1.to_string(),
            create_failing_mock_assertion_verifier().into(),
        )]),
        assertion_attestation_aggregator: Box::new(PassThrough {}),
        ..Default::default()
    };
...
fn client_aggregated_attestation_succeeds() -> anyhow::Result<()> {
    ...
    let client_config = AttestationHandlerConfig {
        ...
        peer_assertion_verifiers: BTreeMap::from([
            (
                MATCHED_ATTESTER_ID1.to_string(),
                create_passing_mock_assertion_verifier(assertion1.clone()).into(),
            ),
            (
                MATCHED_ATTESTER_ID2.to_string(),
                create_passing_mock_assertion_verifier(assertion2.clone()).into(),
            ),
        ]),
...
fn client_one_failed_evidence_verifier_aggregated_attestation_fails() -> anyhow::Result<()> {
    ...
    let client_config = AttestationHandlerConfig {
        ...
        peer_assertion_verifiers: BTreeMap::from([
            ...
            (UNMATCHED_VERIFIER_ID.to_string(), create_failing_mock_assertion_verifier().into()),
        ]),
        assertion_attestation_aggregator: Box::new(All {}),
        ..Default::default()
    };
...
fn client_one_failed_assertion_verifier_aggregated_attestation_fails() -> anyhow::Result<()> {
    ...
    let client_config = AttestationHandlerConfig {
        ...
        peer_assertion_verifiers: BTreeMap::from([
            ...
            (MATCHED_ATTESTER_ID2.to_string(), create_failing_mock_assertion_verifier().into()),
            (UNMATCHED_VERIFIER_ID.to_string(), create_failing_mock_assertion_verifier().into()),
        ]),
        assertion_attestation_aggregator: Box::new(All {}),
        ..Default::default()
...
fn client_unmatched_assertion_verifier_attestation_fails() -> anyhow::Result<()> {
    let client_config = AttestationHandlerConfig {
        peer_assertion_verifiers: BTreeMap::from([(
            UNMATCHED_VERIFIER_ID.to_string(),
            create_passing_mock_assertion_verifier(Assertion::default()).into(),
        )]),
        assertion_attestation_aggregator: Box::new(PassThrough {}),
        ..Default::default()
    };
...
fn pairwise_bidirectional_attestation_succeeds() -> anyhow::Result<()> {
    ...
    let client_config = AttestationHandlerConfig {
        ...
        self_assertion_generators: BTreeMap::from([(
            MATCHED_ATTESTER_ID1.to_string(),
            create_mock_assertion_generator(client_assertion.clone()).into(),
        )]),
        ...
        peer_assertion_verifiers: BTreeMap::from([(
            MATCHED_ATTESTER_ID2.to_string(),
            create_passing_mock_assertion_verifier(server_assertion.clone()).into(),
        )]),
        assertion_attestation_aggregator: Box::new(PassThrough {}),
        ..Default::default()
    };
    let server_config = AttestationHandlerConfig {
        ...
        self_assertion_generators: BTreeMap::from([(
            MATCHED_ATTESTER_ID2.to_string(),
            create_mock_assertion_generator(server_assertion.clone()).into(),
        )]),
        ...
        peer_assertion_verifiers: BTreeMap::from([(
            MATCHED_ATTESTER_ID1.to_string(),
            create_passing_mock_assertion_verifier(client_assertion.clone()).into(),
        )]),
        assertion_attestation_aggregator: Box::new(PassThrough {}),
        ..Default::default()
    };
