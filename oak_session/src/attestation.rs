...
use crate::{config::AttestationHandlerConfig, generator::BindableAssertion, ProtocolEngine};
...
impl AttestationHandler for ClientAttestationHandler {
    ...
    fn take_attestation_state(mut self) -> Result<AttestationState, Error> {
        Ok(AttestationState {
            peer_attestation_verdict: self
                .attestation_result
                .take()
                .ok_or(anyhow!("attestation is not complete"))?,
