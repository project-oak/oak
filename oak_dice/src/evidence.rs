...
static_assertions::assert_eq_size!([u8; 2304], RootLayerEvidence);

/// Private key that can be used by a layer to sign a certificate for the next layer and its
...
static_assertions::assert_eq_size!([u8; 1088], CertificateAuthority);
...
pub struct Stage0DiceData {
    ...
    /// The ECA key and associated certificate for the next layer.
    pub layer_1_ca: CertificateAuthority,
    _padding: [u8; 704],
}

static_assertions::assert_eq_size!([u8; 4096], Stage0DiceData);

/// Keys that can be used for an application for signing or encryption, and their associated
/// certificates.
#[derive(AsBytes, FromBytes)]
#[repr(C)]
pub struct ApplicationKeys {
    /// The RAW bytes representing an ECDSA private key that can be used to sign arbitrary data.
    pub signing_key: [u8; PRIVATE_KEY_SIZE],
...
pub struct RestrictedKernelDiceData {
    ...
    pub root_layer_evidence: RootLayerEvidence,
    /// Certificate representing the identity of the Restricted Kernel
    pub kernel_eca_certificate: [u8; CERTIFICATE_SIZE],
...
static_assertions::assert_eq_size!([u8; 5504], RestrictedKernelDiceData);
