...
fn generate_service(service: &Service) -> anyhow::Result<String> {
    ...
    lines.extend(vec![
        format!("            _ => Err(oak_idl::Error {{ code: oak_idl::ErrorCode::InvalidMethodId, message: None }})"),
        format!("        }}"),
        format!("    }}"),
        format!("}}"),
        format!(""),
...
fn generate_client_method(rpc_call: &RPCCall) -> anyhow::Result<Vec<String>> {
    ...
    Ok(vec![
        ...
        format!(
            "        flatbuffers::root::<{}>(request_body).map_err(|_err| oak_idl::Error {{ code: oak_idl::ErrorCode::InvalidRequest, message: None }})?;",
        ...
        format!("        oak_idl::utils::Message::from_vec(response_body).map_err(|_err| oak_idl::Error {{ code: oak_idl::ErrorCode::InvalidResponse, message: None }})"),
        format!("    }}"),
    ])
}
...
fn generate_server_handler(rpc_call: &RPCCall) -> anyhow::Result<Vec<String>> {
    ...
    Ok(vec![
        ...
        format!(
            "                let request = flatbuffers::root::<{}>(request.body).map_err(|_err| oak_idl::Error {{ code: oak_idl::ErrorCode::InvalidRequest, message: None }})?;",
            request_type(rpc_call),
        ),
        format!(
