...
pub fn compile_services_clients(filename: &str) {
    let schema = FlatbufferSchema::from_file(filename).unwrap();
    let generated_rust_services_clients = generate_from_bytes(&schema.bytes, |service| {
        generate_service_client(service, false)
    })
    .unwrap();
    let out_file = format!(
...
pub fn compile_services_async_clients(filename: &str) {
    let schema = FlatbufferSchema::from_file(filename).unwrap();
    let generated_rust_services_clients = generate_from_bytes(&schema.bytes, |service| {
        generate_service_client(service, true)
    })
    .unwrap();
    let out_file = format!(
...
fn generate_from_bytes<F: Fn(&Service) -> anyhow::Result<String>>(
    schema_bytes: &[u8],
    generator: F,
...
fn generate_client_method(rpc_call: &RPCCall, asynchronous: bool) -> anyhow::Result<Vec<String>> {
    ...
    Ok(vec![
            format!(
                "    pub {}fn {}(&mut self, request_body: Vec<u8>) -> Result<oak_idl::utils::OwnedFlatbuffer<{}>, oak_idl::Status> {{",
