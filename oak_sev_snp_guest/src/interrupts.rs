...
//! Utilities for building interrupt handlers that need to modify the register contents, such as a
//! #VC handler that handles VMM communication exceptions generated by CPUID calls.
//!
//! The "x86-interrupt" calling convention does not allow modification of any general purpose
//! registers, so the #VC handler requires a different approach.
//!
//! Example usage:
//!
//! ```rust
...
/// A reference to the interrupt stack frame will be passed as the first argument of the inner
/// handler function.
///
...
pub struct MutableInterruptStackFrame {
    ...
    pub rsi: u64,
    /// The backed-up value of the instruction pointer when the interrupt happened. Whether this
    /// points to the instruction that caused the interrupt or just after it depends on the
    /// interrupt. For a #VC exception this points to the instruction that caused the exception
    /// (e.g. CPUID).
    pub rip: VirtAddr,
