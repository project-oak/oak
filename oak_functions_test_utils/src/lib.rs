...
pub static MOCK_LOOKUP_DATA_PATH: Lazy<PathBuf> = Lazy::new(|| {
    PathBuf::from(env::var("TEST_SRCDIR").unwrap())
        ...
        .join("oak_functions_launcher")
        .join("mock_lookup_data")
});
...
pub static OAK_RESTRICTED_KERNEL_WRAPPER_BIN: Lazy<PathBuf> = Lazy::new(|| {
    PathBuf::from(env::var("TEST_SRCDIR").unwrap())
        .join(env::var("TEST_WORKSPACE").unwrap())
        .join("oak_restricted_kernel_wrapper")
        .join("oak_restricted_kernel_wrapper_virtio_console_channel_bin")
});
...
pub fn rust_crate_wasm_out_path(crate_name: &str) -> String {
    format!(
        "{}/{}/oak_functions/examples/{crate_name}/{crate_name}.wasm",
        env::var("TEST_SRCDIR").unwrap(),
...
pub fn rust_crate_enclave_out_path(crate_name: &str) -> String {
    format!(
        "{}/{}/enclave_apps/{crate_name}/{crate_name}",
        env::var("TEST_SRCDIR").unwrap(),
        env::var("TEST_WORKSPACE").unwrap(),
    )
}
...
pub fn run_java_client(addr: &str) -> std::io::Result<std::process::Output> {
    std::process::Command::new(
        PathBuf::from(env::var("TEST_SRCDIR").unwrap())
            .join(env::var("TEST_WORKSPACE").unwrap())
            .join("java/src/main/java/com/google/oak/client/oak_functions_client")
