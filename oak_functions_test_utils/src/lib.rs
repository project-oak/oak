...
/// Builds the crate identified by the given package name (as per the `name`
/// attribute in a Cargo.toml file included in the root cargo workspace) as a
/// Linux binary, and returns the path of the resulting binary.
pub fn build_rust_crate_linux(crate_name: &str) -> anyhow::Result<String> {
    ...
    Ok(format!("{}target/x86_64-unknown-linux-musl/release/{crate_name}", env!("WORKSPACE_ROOT")))
}
