...
impl Machine for I440fx {
    ...
    fn mmio64_hole<P: Platform>(
        ...
        I440fx::mmio64_hole_helper(firmware, zero_page, 41)
    }
}
...
impl Machine for IntelVirtPcieHost {
    ...
    fn mmio64_hole<P: Platform>(
        ...
        // trick for I440fx should be okay, but since CH respects the CPUID phys address, we
        // can use the max page size
        // https://github.com/cloud-hypervisor/cloud-hypervisor/blob/main/arch/src/x86_64/layout.rs#L124-L125
        // So, no need for anything special here.
        I440fx::mmio64_hole_helper(firmware, zero_page, P::guest_phys_addr_size())
    }
}
...
mod tests {
    ...
    fn IntelVirtPcieHost_hole() {
        ...
        assert_that!(
            ...
            ok(all!(
                field!(&Range.start, eq(0xC000_0000)),
                field!(&Range.end, eq(0xE800_0000))
            ))
        )
    }
