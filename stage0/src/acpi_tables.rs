...
// GAS for short. This is a structure used to describe the position of registers.
...
pub struct Fadt {
    ...
    // Reserved bits. Populated in ACPI 1.0, but removed in ACPI 2.0. Remains for compatibility only
    _reserved1: u8,
...
// Make this comparison against the ACPI 6.0 spec. In validate() we will account for
// older revisions
...
impl Fadt {
    ...
    fn validate(&self) -> ResultStaticErr<()> {
        ...
        // Depending on the major version provided in the FADT, this is subject to change.
        // We adjust the check based on the major version
        let acpi_6_length = size_of::<Fadt>() as u32;
        let expected_table_size =
        if self.header.revision == 6 {
            acpi_6_length
        } else if self.header.revision == 5 {
            acpi_6_length - size_of::<u64>() as u32
        } else if self.header.revision < 5 {
            acpi_6_length - size_of::<u64>() as u32 - 2 * size_of::<GenericAddressStructure>() as u32
        } else {
            return Err("Unexpected ACPI major version");
        };

        if self.header.length != expected_table_size {
            log::error!("FADT header length did not match the expected table struct size. Expected: {}, got: {:#x}", expected_table_size, {self.header.length});
