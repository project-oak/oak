...
impl Decodable for EpitaphBody {
    fn new_empty() -> Self {
        Self {
            error: zx_status::Status::new_empty(),
        }
    }
    fn decode(&mut self, decoder: &mut Decoder<'_>) -> Result<()> {
...
impl<T: Autonull> Decodable for Option<Box<T>> {
    ...
    fn decode(&mut self, decoder: &mut Decoder<'_>) -> Result<()> {
        if T::naturally_nullable(&decoder.context) {
            ...
            if present {
                self.get_or_insert_with(|| Box::new(T::new_empty()))
                    .decode(decoder)
            } else {
                *self = None;
            ...
            match present {
                ALLOC_PRESENT_U64 => {
                    decoder.read_out_of_line(decoder.inline_size_of::<T>(), |decoder| {
                        self.get_or_insert_with(|| Box::new(T::new_empty()))
                            .decode(decoder)
                    })
                }
...
mod test {
    ...
    fn decode_fails_without_reserved_table_fields() {
        ...
        for ctx in CONTEXTS {
            ...
            let mut table = TableWithoutGaps {
                first: Some(1),
                second: Some(2),
            };
            let buf = &mut Vec::new();
    ...
    fn encode_decode_transaction_msg() {
        for ctx in CONTEXTS {
            ...
            let start = &mut TransactionMessage {
                header,
                body: &mut body.clone(),
            };
