...
use cleanroom_lib::{client, ifc, policy, protocol, server, shell};
...
enum Command {
    ...
    Server {
        ...
        ///
        /// Defines environment variable labels and module
...
fn main() {
    ...
    let result = match args.command {
        Command::Run { module, socket } => client::client_run(&socket, &module),
        Command::Ls { socket } => client::client_list(&socket),
        Command::Shell(ref shell_args) => shell::run_shell(shell_args),
        Command::Server { policy_file, cells_file, socket, module_store } => {
            server::run_server(&policy_file, cells_file.as_deref(), &socket, &module_store)
        }
        Command::RunLocal { wasm_module_file, policy_file } => {
            run_local(&wasm_module_file, policy_file.as_deref())
        }
    };
...
fn run_local(
    ...
    let pc = Arc::new(Mutex::new(ifc::ComputationLabel::new(ifc::Label::public())));
    let mut stdin = Vec::new();
    use std::io::Read;
    std::io::stdin().read_to_end(&mut stdin).context("reading stdin")?;

    let (status, _label, stdout, stderr) = server::run_component_module(
        &wasm_bytes,
        &policy,
        &stdin,
        &pc,
        ifc::DeclassificationPrivilege::none(),
        None,
    )?;
