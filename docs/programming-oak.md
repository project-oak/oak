...
represent the Node itself, and then add the `derive(OakGrpcEntrypoint)`
attribute (from the
[`oak_derive`](https://project-oak.github.io/oak/sdk/oak_derive/index.html)
crate):

<!-- prettier-ignore-start -->
[embedmd]:# (../examples/rustfmt/module/rust/src/lib.rs Rust /.*derive\(OakGrpcEntrypoint\).*]/ /;$/)
```Rust
...
<!-- prettier-ignore-start -->
[embedmd]:# (../examples/running_average/module/rust/src/lib.rs Rust /.*derive\(Default, OakGrpcEntrypoint\).*]/ /^}/)
```Rust
...
[`derive(OakGrpcEntrypoint)`](https://project-oak.github.io/oak/sdk/oak_derive/derive.OakGrpcEntrypoint.html)
...
- Create an instance of your Node `struct` (using the `new()` method from the
  [`OakGrpcServerNode`](https://project-oak.github.io/oak/sdk/oak/grpc/trait.OakGrpcServerNode.html)
...
- A
  [`new()`](https://project-oak.github.io/oak/sdk/oak/grpc/trait.OakGrpcServerNode.html#tymethod.new)
...
<!-- prettier-ignore-start -->
[embedmd]:# (../examples/rustfmt/module/rust/src/lib.rs Rust /impl OakGrpcServerNode/ /^}/)
```Rust
...
implementation of the `OakGrpcServerNode::invoke()` method described in the
previous section.
...
- This Node `struct` implements `oak::grpc::OakGrpcServerNode` so the
  `event_loop()` method can call back into the Node's `invoke()` method.
...
- A separate thread running an inner main entrypoint for the Node is started
  ...
  `oak_runtime::NodeMain`, and the `derive(OakGrpcEntrypoint)` macro (from the
  [`oak_derive`](https://project-oak.github.io/oak/sdk/oak_derive/index.html)
