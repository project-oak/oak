...
A client can then create a **room**, which generates and returns a pair of
random bearer tokens:

- the **room access token** is needed to join that room and participate in chats
  there; this is printed (as a base-64 string) to allow sharing, and it is used
  by the client for two purposes:
  - label messages sent to the chat room with a confidentiality label
    corresponding to the token, so that only other clients that are able to
    authenticate with the same token may read them
  - authenticate to the server, so that it may read messages for the same room
    sent by other clients
- the **room admin token** allows destruction of the room instance at the
  server; the client keeps hold of this token internally.
...
```bash
./scripts/runner --logs run-examples --run-server=false --example-name=chat --client-additional-args=--test=false --client-additional-args=--room_access_token=NKsceNlg69UbcvryfzmFGnMv9qnZ0DYh6u6gJxujnPPxvHsxMehoD368sumKawVaq9WaSkzrcStoNYLvVNdzhA==
```
