...
oak::entrypoint!(oak_main<ConfigMap> => |receiver: Receiver<ConfigMap>| {
    ...
    let (init_sender, init_receiver) = oak::io::channel_create::<AggregatorInit>("Aggregator initial channel", &Label::public_untrusted())
    ...
    let (invocation_sender, invocation_receiver) = oak::io::channel_create::<grpc::Invocation>("gRPC invocation channel", &Label::public_untrusted())
