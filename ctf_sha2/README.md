...
Until recently, the attestation verification logic did not inspect the JWT's
claims to verify that the token originated from a production environment. This
created a significant hole that an attacker could exploit to falsify the claim
of this binary.
...
2. **Normal Execution**: The `ctf_sha2` binary starts, generates a random flag,
   computes its digest, and successfully requests a valid attestation JWT
   containing the digest as a nonce. It prints this JWT to the logs and then
   exits.
...
4. **Extract the Flag**: Because the VM is running a debug image, it does not
   terminate when the workload exits. The attacker uses the debug feature to
   **SSH into the running VM**. Although the `ctf_sha2` process has terminated,
   its memory pages may not have been zeroed out by the OS. The attacker can
   then use memory forensics tools to scan the VM's memory and find the 64-byte
   flag, which is still present.

This specific vulnerability has been fixed, but it serves as a powerful example
of the types of thinking required to break TEE-based claims.
