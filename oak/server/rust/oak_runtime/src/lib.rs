...
impl OakRuntime {
    ...
    pub fn node_started(&mut self, node_name: &str, join_handle: std::thread::JoinHandle<()>) {
        self.nodes
            .get_mut(node_name)
    ...
    pub fn stop_next(&mut self) -> Option<(String, std::thread::JoinHandle<()>)> {
...
fn log_node_main(handle: Handle) {
    if handle == oak_abi::INVALID_HANDLE {
        error!("Invalid handle provided to log pseudo-Node");
        return;
    }
    ...
    loop {
        match oak::wait_on_channels(&[half]) {
            Err(OakStatus::ERR_TERMINATED) => {
                info!("log pseudo-Node terminated");
                return;
            }
            Err(s) => {
                error!("failed to wait for message, {:?}", s);
                return;
            }
            Ok(_) => {}
        }
