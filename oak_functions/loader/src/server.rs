...
use crate::{logger::Logger, LookupData};
use anyhow::Context;
...
impl WasmInterface {
        ...
        if source.len() as u32 <= dest_capacity {
            self.get_memory().set(dest, source).map_err(|err| {
                self.logger.log_sensitive(
                    Level::Error,
                    &format!(
                        "Unable to write destination buffer into guest memory: {:?}",
                        err
                    ),
                );
    ...
    pub fn read_request(
        ...
        self.write_to_wasm_memory(&self.request_bytes, dest, dest_capacity)
    }
    ...
    pub fn storage_get_item(
        ...
        value_actual_size: AbiPointerOffset,
    ) -> Result<(), OakStatus> {
        let key = self
            .get_memory()
            ...
            .map_err(|err| {
                self.logger.log_sensitive(
                    Level::Error,
                    &format!(
                        "storage_get_item(): Unable to read name from guest memory: {:?}",
                        err
                    ),
                );
                OakStatus::ErrInvalidArgs
