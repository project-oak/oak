...
pub struct Response {
    ...
    /// The body of the response generated by the Wasm module, in case of success. Or the reason of
    /// the failure in case of an error. It may contain a number of trailing 0s.
    pub body: alloc::vec::Vec<u8>,
...
const RESPONSE_LENGTH_OFFSET: usize = 0;
...
const RESPONSE_STATUS_CODE_OFFSET: usize = 8;
static_assertions::assert_eq_size!([u8; RESPONSE_STATUS_CODE_SIZE], StatusCode);

const RESPONSE_BODY_OFFSET: usize = 16;
    ...
    pub fn decode(bytes: &[u8]) -> anyhow::Result<Self> {
        ...
        let status: StatusCode = {
            ...
            StatusCode::from_repr(u32::from_le_bytes(status_bytes))
                .ok_or(anyhow::Error::msg("Invalid status code"))?
        };
        let mut body: Vec<u8> =
            Vec::with_capacity(bytes.len() - RESPONSE_LENGTH_SIZE - RESPONSE_STATUS_CODE_SIZE);

        body.copy_from_slice(&bytes[RESPONSE_BODY_OFFSET..]);
