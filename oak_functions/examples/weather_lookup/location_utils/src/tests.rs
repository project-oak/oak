...
fn test_find_cell() {
    ...
    // Polar circle.
    // Near 45 degreees.
    let polar = find_cell(-89.2, 57.1).unwrap().index;
...
fn test_distance_two_points_same_cell() {
    ...
    // This should be close enough to be rounded to the same value meters.
    assert_eq!(distance, hav_distance);
}
...
fn test_distance_two_points_different_cell() {
    ...
    // Allow up to 10m distortion from the projection at this larger distance.
