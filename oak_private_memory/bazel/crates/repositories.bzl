...
load("@oak//bazel/rust:defs.bzl", "RUST_NIGHTLY_VERSION")
    ...
    aliasing_crates_repository(
        ...
        repositories = [
            aliased_crates_repository(
                name = "oak_no_std_crates_index",
                conditions = [Label("@oak//:x86_64-none-setting"), Label("@oak//:wasm32-none-setting")],
                packages = OAK_NO_STD_CRATES.keys() + extra_no_std_packages.keys(),
                # See comment on micro_rpc/BUILD.
                overrides = {"prost-types": Label("@oak//third_party/prost-types")},
            ),
            aliased_crates_repository(
                name = "oak_no_std_no_avx_crates_index",
                conditions = [Label("@oak//:x86_64-none-no_avx-setting")],
