...
// TODO(#3640): Merge this evidence and the evidence proto from the `messages.proto`.
...
/// Attestation report generated by the Trusted Execution Environment.
...
// TODO(#3640): Rename to AttestationEvidenceGenerator.
...
pub trait AttestationReportGenerator: Clone + Send + Sync {
...
pub trait AttestationReportVerifier: Clone + Send + Sync {
...
impl AttestationReportVerifier for EmptyAttestationVerifier {
    fn verify_attestation_evidence(
        &self,
        attestation_report: &AttestationReport,
        ...
        // attestations.
        if attestation_report.value.is_empty() {
            Ok(())
        } else {
            Err(anyhow::anyhow!(
