...
/**
 * Verifier class providing functions for verifying a Transparent-Release endorsement statement, and
...
public class EndorsementVerifier {
   ...
   * <ul>
   * <li> verifying the signature in {@code signedEntryTimestamp} (retrieved from {@code
   * logEntryBytes}), using Rekor's public key ({@code pemEncodedRekorPublicKeyBytes}).
   * <li> verifying the signature in {@code body.RekordObj.signature} (retrieved from {@code
   * logEntryBytes}), using the endorser's public key (also retrieved from {@code logEntryBytes}).
   * <li> verifying that the content of the body (retrieved from {@code logEntryBytes}) matches the
   * input {@code endorsementStatementBytes}.
   * </ul>
   * @param logEntryBytes
   * @param pemEncodedRekorPublicKeyBytes
   * @param endorsementStatementBytes
   * @return an empty Result if the verification succeeds, or an Exception wrapped in a Result
   *     otherwise
   */
  public Optional<Exception> verifyRekorLogEntry(byte[] logEntryBytes,
    ...
    return verifyRekorSignature(logEntryBytes, pemEncodedRekorPublicKeyBytes)
        .andThen(RekorLogEntry.getRekorLogEntryBody(logEntryBytes))
  ...
  /**
   * Verifies the integrity of the Rekor body, by verifying the signature in the {@code body} over
   * the given {@code contentsBytes}, using the public key in the {@code body}. This public is
   ...
   *     otherwise
   */
  public Result<Void, Exception> verifyRekorBody(Body body, byte[] contentBytes) {
    ...
    // Check that hash of the given content matches the hash of the data in the Body.
    let contentsHashHex = sha256Hex(contentBytes);
    ...
    return verifySignature(body.spec.signature.content, contentsBytes, pemEncodedPublicKeyBytes);
  }
  ...
  /**
   * Verifies the given base64-encoded signature over the given content bytes, using the given
